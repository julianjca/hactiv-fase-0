...
function trainSeating (penumpang) {
  ...
  for(let i = 0;i<penumpang.length;i++){
    let simpan = {};
    simpan.nama = penumpang[i].nama;
    simpan.seat = penumpang[i].seat;

    if(penumpang[i].gerbong === "VVIP"){
      result.VVIP.push(simpan);
    }
    if(penumpang[i].gerbong === "VIP"){
      result.VIP.push(simpan);
    }
    if(penumpang[i].gerbong === "Regular"){
      result.Regular.push(simpan);
