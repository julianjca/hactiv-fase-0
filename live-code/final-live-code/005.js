...
function ganjilGenapAsianGames ( date, data ) {
  ...
  if(date<1||date>31){
    ...
    for(let i = 0;i<data.length;i++){
      let newArr = data[i].plat.split(' ');
      if(date%2===0&&(parseInt(newArr[1][newArr[1].length-1])%2!==0&&data[i].type==='Mobil')){
        counter++;
      }
      else if(date%2!==0&&(parseInt(newArr[1][newArr[1].length-1])%2===0&&data[i].type==='Mobil')){
        counter++;
      }
    }
  }
