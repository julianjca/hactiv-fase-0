...
function invokeSpell (input) {
  ...
  let condition;
  condition = input.includes("a","b","c","d","e","f","h","i","j","k","l","m","n","o","p","r","s","t","u","v","x","y","z");

  if(condition===true){
    return "Combination does not exist";
  } else{
    let charMap = {q:0,w:0,e:0};
  ...
  for(let i = 0;i<spells.length;i++){
    if(charMap.q===spells[i].q &&charMap.w===spells[i].w && charMap.e===spells[i].e){
      result = spells[i].spellName;
    }
  }
  return result;
  }
